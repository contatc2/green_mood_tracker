...
def generate_prediction(data, model_name=MODEL_NAME, model_version=MODEL_VERSION, download_files=True, model=None):
    if download_files:
    ...
    if model_name == 'RoBERTa':
        ...
        y_pred = np.array(results).reshape((len(data), 2))
    else:
        encoder = Word2VecEncoder()
        X = data.text
        y_pred = model.predict(encoder.transform(X))
    print(colored(y_pred.shape, 'green'))
    print(colored(y_pred[0], 'red'))
    return y_pred


def twint_prediction(data_filename, model_name=MODEL_NAME, model_version=MODEL_VERSION, download_files=False, model=None):
    data = get_prediction_data(data_filename)
    y_pred = generate_prediction(
        data, model_name=model_name, model_version=model_version, download_files=download_files, model=model)
...
if __name__ == '__main__':
    ...
    data = get_test_data()
    generate_prediction(data, model_name=MODEL_NAME, model_version='v0', download_files=False, model=None)
